...
export default function Poll(props) {
  ...
  const [chart, setChart] = useState(<DataChart data={props.data}></DataChart>);
    ...
    for (let i = 0; i < output.options.length; i++) {
      ...
      for (let j = 0; j < output.options[i].voters.length; j++) {
        ...
        for (let k = 0; k < filterList2.length; k++) {
          if (
            filterList2[k] == output.options[i].voters[j].info.gender &&
            !voted
          ) {
            count++;
          }
  ...
  function copyToClipboard() {
    ...
    tempInput.value = `${process.env.NEXT_PUBLIC_URL}/vote${props.url.slice(
      5
    )}`;
    ...
    <div>
      ...
      <div className={styles.main}>
        ...
        <div
          ...
          className={copyStyles.linkDisplayContainer}
        >
          <div className={copyStyles.linkHolder}>
            <p id="voteLink">{`${
              process.env.NEXT_PUBLIC_URL
            }/vote${props.url.slice(5)}`}</p>
          </div>
